...
export function CallProvider({ children }: { children: React.ReactNode }) {
    ...
    const initPeer = async () => {
      ...
      import('peerjs').then(({ default: PeerClass }) => {
        ...
        newPeer.on('call', async (call: any) => {
          ...
          // Check if it's a video call based on metadata
  ...
  const attachStream = (stream: MediaStream, isLocal: boolean) => {
    if (callType === 'video') {
      ...
      } else if (!isLocal && remoteAudioRef.current) {
  ...
  const stopMediaTracks = () => {
    [localVideoRef, localAudioRef].forEach(ref => {
  ...
  const startCall = async (targetUserId: string, targetUserName: string, type: CallType) => {
    if (!peer) {
      alert("Call system is disconnected. Please refresh the page.");
      return;
    }
    try {
      setIsCalling(true);
          ...
          >
             ...
             >
                ...
                <div className="flex w-full gap-4">
                  <button 
                    onClick={rejectCall}
                    className="flex-1 rounded-xl bg-red-500/20 text-red-500 hover:bg-red-500/30 font-bold py-4 transition-colors"
                  >
                    Decline
                  </button>
                  <button 
                    onClick={answerCall}
                    className="flex-1 rounded-xl bg-emerald-500/20 text-emerald-500 hover:bg-emerald-500/30 font-bold py-4 transition-colors flex items-center justify-center gap-2"
                  >
      ...
      <AnimatePresence>
        {(isCalling || activeCall) && callType === 'audio' && (
          <motion.div 
            ...
            className="fixed top-6 left-1/2 -translate-x-1/2 z-[9998] flex items-center gap-4 rounded-full border border-[var(--border-color-strong)] bg-black/90 px-6 py-3 shadow-2xl backdrop-blur-md min-w-[280px]"
          >
             <div className="flex items-center gap-3 w-full">
                ...
                <div className="flex flex-col flex-1">
                   <span className="text-sm font-bold text-white truncate max-w-[120px]">{remoteUser?.name || 'Unknown'}</span>
                ...
                <button 
                  onClick={endCall}
                  className="ml-auto flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-red-500 text-white hover:bg-red-600 transition-colors shadow-lg"
                >
        ...
        {(isCalling || activeCall) && callType === 'video' && (
          ...
          >
            ...
            <div className="absolute bottom-0 left-0 right-0 p-8 flex justify-center items-center gap-6 z-20 bg-gradient-to-t from-black/80 to-transparent">
              <button 
                onClick={endCall}
                className="flex h-16 w-16 items-center justify-center rounded-full bg-red-500 text-white hover:bg-red-600 transition-colors shadow-2xl hover:scale-110 active:scale-95"
              >
